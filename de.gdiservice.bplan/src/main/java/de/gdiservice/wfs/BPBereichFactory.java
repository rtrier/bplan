...
public class BPBereichFactory implements WFSFactory<BPBereich>  {
    
    final static Logger logger = LoggerFactory.getLogger(BPBereichFactory.class);
    

	@Override
	public BPBereich build(SimpleFeature f) throws IOException {
		
		ObjectReader objectReader = new ObjectMapper().reader();
		
		final BPBereich bereich = new BPBereich();
...
//	    LocalDate versionbaunvodatum;
        bereich.setVersionbaunvodatum( WFSFactory.getAsLocalDate(f, "versionbaunvodatum"));        
//	    String versionbaugbtext;
        bereich.setVersionbaugbtext( WFSFactory.getAsString(f, "versionbaugbtext") );
//	    String versionsonstrechtsgrundlagetext;
        bereich.setVersionsonstrechtsgrundlagetext( WFSFactory.getAsString(f, "versionsonstrechtsgrundlagetext") );
//	    String versionbaunvotext;
        bereich.setVersionbaunvotext( WFSFactory.getAsString(f, "versionbaunvotext") );
//	    LocalDate versionsonstrechtsgrundlagedatum;
        bereich.setVersionsonstrechtsgrundlagedatum( WFSFactory.getAsLocalDate(f, "versionsonstrechtsgrundlagedatum"));
//	    LocalDate versionbaugbdatum;
        bereich.setVersionbaugbdatum( WFSFactory.getAsLocalDate(f, "versionbaugbdatum"));
//	    UUID gehoertzuplan;		
