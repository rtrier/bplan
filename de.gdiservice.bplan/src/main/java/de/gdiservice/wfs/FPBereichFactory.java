...
public class FPBereichFactory implements WFSFactory<FPBereich>  {
    
    final static Logger logger = LoggerFactory.getLogger(FPBereichFactory.class);
    

	@Override
	public FPBereich build(SimpleFeature f) throws IOException {
		
		ObjectReader objectReader = new ObjectMapper().reader();
		
		final FPBereich bereich = new FPBereich();
